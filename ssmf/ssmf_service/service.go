...
func SessionReq(c *gin.Context) { //TODO: Change input data 'data' to appropriate attribute
	...
	log.Print("##############################\n")
	log.Print("2. Path Selection Process\n")
	/*
		Add Path Selection Code
	*/
	log.Print("##############################\n")
	log.Print("##############################\n")
	log.Print("3. Security Process\n")
	/*
		Add Security Process Code
	*/
	log.Print("##############################\n")
	log.Print("4. Table Update Request\n")
	log.Print("##############################\n")
	...
	resp, err := http.Post("http://localhost:24247/smn-service/v1/TableUpdate", "application/json", bytes.NewBuffer([]byte(jsonStr)))
	if err != nil {
...
func TableUpdate(c *gin.Context) { //TODO: Change input data 'data' to appropriate attribute
	...
	*/
	...
	http := &http.Client{Transport: transport}
	resp, err := http.Post("http://localhost:24243/smn-service/v1/ContextTransfer", "application/json", bytes.NewBuffer([]byte(jsonStr)))
	if err != nil {
		fmt.Println("error: %v", err)
	} else {
		fmt.Println(resp.Header)
...
func NotificationData(c *gin.Context) { //TODO: Change input data 'data' to appropriate attribute
	...
	*/
	...
	resp, err := http.Post("http://localhost:24243/smn-service/v1/DataTransferReq", "application/json", bytes.NewBuffer([]byte(jsonStr)))
	if err != nil {
		fmt.Println("error: %v", err)
	} else {
		fmt.Println(resp.Header)
