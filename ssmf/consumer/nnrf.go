...
func BuildNFInstance(context *ssmf_context.SSMFContext) models.NfProfile {
	var profile models.NfProfile
	config := factory.SsmfConfig
	profile.NfInstanceId = context.NfId
	profile.NfType = models.NfType_SMF
...
func SendRegisterNFInstance(nrfUri, nfInstanceId string, profile models.NfProfile) (string, string, error) {
	...
	for {
		...
		if err != nil || res == nil {
			// TODO : add log
			fmt.Println(fmt.Errorf("SSMF register to NRF Error[%s]", err.Error()))
...
func SendDeregisterNFInstance() (problemDetails *models.ProblemDetails, err error) {
	...
	ssmfSelf := ssmf_context.SSMF_Self()
	// Set client and set url
	configuration := Nnrf_NFManagement.NewConfiguration()
	configuration.SetBasePath(ssmfSelf.NrfUri)
	client := Nnrf_NFManagement.NewAPIClient(configuration)

	var res *http.Response

	res, err = client.NFInstanceIDDocumentApi.DeregisterNFInstance(context.Background(), ssmfSelf.NfId)
	if err == nil {
		return
	} else if res != nil {
		defer func() {
...
// 		NfInstanceId:  ssmf_context.SSMF_Self().NfInstanceID,
// 		NfType:        "SSMF",
// 		NfStatus:      models.NfStatus_REGISTERED,
// 		Ipv4Addresses: []string{ssmf_context.SSMF_Self().RegisterIPv4},
// 	}
...
// 			RegisterNFInstance(context.TODO(), ssmf_context.SSMF_Self().NfInstanceID, profile)
// 		if err != nil || res == nil {
// 			logger.ConsumerLog.Infof("SSMF register to NRF Error[%s]", err.Error())
// 			time.Sleep(2 * time.Second)
// 			continue
// 		}
// 		defer func() {
...
// 			ssmf_context.SSMF_Self().NfInstanceID = resourceUri[strings.LastIndex(resourceUri, "/")+1:]
// 			break
// 		} else {
...
// 	logger.InitLog.Infof("SSMF Registration to NRF %v", rep)
// 	return nil
// }
...
// 	return fmt.Errorf("[SSMF] Retry NF Registration has meet maximum")
// }

// func SendNFDeregistration() error {
// 	// Check data (Use RESTful DELETE)
// 	res, localErr := ssmf_context.SSMF_Self().
// 		NFManagementClient.
// 		NFInstanceIDDocumentApi.
// 		DeregisterNFInstance(context.TODO(), ssmf_context.SSMF_Self().NfInstanceID)
// 	if localErr != nil {
